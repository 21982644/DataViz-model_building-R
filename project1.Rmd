...
filtered <- within(filtered, {
     ...
     RETURN_TO_WORK_DT<- as.POSIXct(RETURN_TO_WORK_DT, format="%d/%m/%Y")
})
```
...
p10 <- ggplot(data = filtered, aes(x=WKDAY)) + 
       geom_bar(colour="black", fill="steelblue") +
       labs(title = "Accident by Week", x="Week" ,y= "Frequency")

p11 <- ggplot(data = filtered, aes(x=ACCIDENT_TIME_NEW)) + 
...
Let's look the shift time to see if more shifts are during the day. The compare the distribution between shift and accident time at differnet time point, I'll use two density plots here. 
```{r message=FALSE, warning=FALSE}
ggplot(filtered) + 
...
nona <- na.omit(filtered$DAY_AWY) 
length(nona[nona < 0])
```
Interestingly,there are some negative days when we calculate the days between accidents date happened and the date return to work. Clearly, there is a problem with the format of the date.The most likely reason would be they switched between month/day/yr to day/month/yr format either in the accidents date or the return to work date. And the DAY_AWY column doesn't really match with DAY_LOST column. 
...
ggplot(tmp, 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
We can see permanent injuries are mostly found in CGHT over exertion group, and struck.

### Coal/Metal
Segmented bar plot to look the proportion of 
```{r message=FALSE, warning=FALSE}
atmine <- filtered %>%
...
ggplot(atmine, 
  ...
  coord_flip()
```
All groups except no injuries accidents, more accidents are found on metal mine as compared to coal mine. Let's look the percentage of coal vs metal mines.
```{r message=FALSE, warning=FALSE}
tmp <- filtered %>%
...
### Activity 
There are too much information in the activity column and it's hard to re-group them to make a clear and viewable graph, so a world cloud that indicates the frequency of words will be used here to give us a brief overview of the activities.  
